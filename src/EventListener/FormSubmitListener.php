...
final readonly class FormSubmitListener
{
    ...
    public function onFormSaved(FormSavePostEvent $event): void
    {
        ...
        if ($contact instanceof Contact) {
            ...
            $accountMail = $this->onboardContact($contact, $email);
        }
        ...
        if ($accountMail instanceof Mail) {
            $this->mailer->send($accountMail);
        }
    }
    ...
    private function onboardContact(Contact $contact, string $email): Mail
    {
        ...
        $user
            ->setContact($contact)
            ->setPasswordResetToken($this->hashResetToken($token))
